...
(function(){
  ...
  window.SpoodAccount = {
    ...
    init: function(sb){
      ...
      nudge.className = 'acct-nudge'; nudge.hidden = true;
      nudge.innerHTML = '<span>One last step: pick your spood-sender name</span><button type="button">Pick my name</button>';
      document.body.appendChild(nudge);
      nudge.querySelector('button').addEventListener('click', function(){ open(); });

      function emit(){
        nudge.hidden = !(state.ready && state.user && !state.profile);
